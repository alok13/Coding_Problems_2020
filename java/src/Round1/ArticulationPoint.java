...
public class ArticulationPoint {
    ...
    private static void dfs(Map<Integer, Set<Integer>> map, int[] low, int[] ids, int[] parent, int cur, Set<Integer> res) {
        ...
        for (int child_Node : map.get(cur)) {
            if (ids[child_Node] == -1) {
                children++;
                parent[child_Node] = cur;
                dfs(map, low, ids, parent, child_Node, res);
                low[cur] = Math.min(low[cur], low[child_Node]);
                if ((parent[cur] == -1 && children > 1) || (parent[cur] != -1 && low[child_Node] >= ids[cur]))
                    res.add(cur);
            } else if (child_Node != parent[cur])
                low[cur] = Math.min(low[cur], ids[child_Node]);
        }
    }
}
